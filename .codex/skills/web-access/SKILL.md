...
# Codex Windows 适配：以下所有 shell 命令在 PowerShell 中执行
# 定义 SKILL_DIR 为当前 skill 根目录，方便脚本引用
# 用户若希望全局安装，请将此 skill 整个目录移至 $env:CODEX_HOME\skills\web-access
...
按脚本输出处理：
- `exit 0` → 继续
- `exit 2` → 需询问用户偏好，写入 `$env:SKILL_DIR\config.env` 的 `WEB_ACCESS_BROWSER`
...
切换浏览器时，proxy 是长驻进程，需先 `Get-Process -Name node -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -match 'cdp-proxy' } | Stop-Process -Force` 再重跑 check-deps。
...
```bash
node "$env:SKILL_DIR/scripts/find-url.mjs" [关键词...] [--only bookmarks|history] [--browser chrome|edge] [--limit N] [--since 1d|7h|YYYY-MM-DD] [--sort recent|visits]
```
...
# 捕获页面渲染状态（含视频当前帧）
curl.exe -s "http://localhost:3456/screenshot?target=ID&file=/tmp/shot.png"
